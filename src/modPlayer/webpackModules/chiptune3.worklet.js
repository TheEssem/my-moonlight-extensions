...
class MPT extends AudioWorkletProcessor {
    ...
    switch (msg.data.cmd) {
      ...
      case "setPitch":
        ...
        libopenmpt._openmpt_module_ctl_set(
          this.modulePtr,
          asciiToStack("play.pitch_factor"),
          asciiToStack(v.toString())
        );
        break;
      case "setTempo":
        if (!libopenmpt.stackSave || !this.modulePtr) return;
        libopenmpt._openmpt_module_ctl_set(
          this.modulePtr,
          asciiToStack("play.tempo_factor"),
          asciiToStack(v.toString())
        );
        break;
      case "selectSubsong":
  ...
  getSong() {
    ...
    // patterns
    for (
      let patIdx = 0, patNum = libopenmpt._openmpt_module_get_num_patterns(this.modulePtr);
      patIdx < patNum;
      patIdx++
    ) {
      const pattern = {
      ...
      // rows
      for (
        let rowIdx = 0, rowNum = libopenmpt._openmpt_module_get_pattern_num_rows(this.modulePtr, patIdx);
        rowIdx < rowNum;
        rowIdx++
      ) {
        const row = [];
        ...
        for (let chIdx = 0; chIdx < chNum; chIdx++) {
          ...
          for (let comIdx = 0; comIdx < 6; comIdx++) {
            /* commands
						...
						*/
            channel.push(
              libopenmpt._openmpt_module_get_pattern_row_channel_command(this.modulePtr, patIdx, rowIdx, chIdx, comIdx)
            );
          }
          row.push(channel);
        }
